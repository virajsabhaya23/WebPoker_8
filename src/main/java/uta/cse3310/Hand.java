...
public class Hand {
	...
	public Card[] sort(Hand hnd) {
		...
		for (i = 0; i < 5; i++) {
			int min_ind = i;
			for (j = i + 1; j < 5; j++) {
				if (Arrays.asList(this_hand_enum_values).indexOf(hnd.cards[j].value) > Arrays.asList(this_hand_enum_values)
	...
	public boolean is_better_than(Hand H) {
		...
		// System.out.println("H1 High Card" + " " + this.highCard + " " + "H1 Hand
		// Ranking" + " " + this.ranking); // Ranking and HighCard
		// System.out.println("H2 High Card" + " " + H.highCard + " " + "H2 Hand
		// Ranking" + " " + H.ranking); // Ranking and HighCard
